...
public:
    Clustering() {
        sub_ = nh_.subscribe(nh_.getNamespace() + "/no_ground_cloud", 1, &Clustering::cloudCallback, this);
...
private:
    void cloudCallback(const sensor_msgs::PointCloud2ConstPtr& input) {
        ...
        ec.setClusterTolerance(cluster_tolerance); // 20cm
        ...
        for (const auto& indices : cluster_indices) {
            ...
            Eigen::Vector3f centroid = accumulator;
        ...
        ROS_INFO("Euclidian clustering took %f ms", elapsed_time.count()*1000.0f);
    }
